...
# from schemas import ProdStat as ModelSchema
...
#     for x in range(0, 15):
#         await Model.create(api10=rand_str())
...
#         target=uvicorn.run,
#         # args=(app,),
#         args=("tests.collector.test_requestor:app",),
...
# # async def test_server(server, seed_users):
...
# @pytest.fixture
# def endpoint():
#     yield Endpoint("model", Model)


# @pytest.fixture
# def requestor(endpoint, response_body):
...
#     async def test_iter_links(self, server, seed_users):
...
#     # TODO: same problem
#     async def test_iter_from_body_counts(self, server, seed_users):
#         requestor = AsyncClient(base_url=server)
#         responses = []
...
#     async def test_iter_from_body_urls(self, server, seed_users):
#         requestor = AsyncClient(base_url=server)
#         responses = []
#         async for r in requestor.iter_pages(
...
#     async def test_iter_from_body_total_pages(self, server, seed_users):
#         requestor = AsyncClient(base_url=server)
#         responses = []
#         async for r in requestor.iter_pages(
...
#     async def test_iter_pages_with_extra_params(self, server, seed_users):
#         requestor = AsyncClient(base_url=server)
#         responses = []
#         async for r in requestor.iter_pages(
#             "/paged/body/counts",
...
#     async def test_iter_pages_no_content(self, server, seed_users):
#         requestor = AsyncClient(base_url=server)
#         responses = []
#         async for r in requestor.iter_pages(
#             "/no-content",
...
# class TestRockRequestor:
#     async def test_authenticate_with_cookie(self, response_body):
#         cookie = {
#             "name": ".ROCK",
#             "value": "D7D6322A952",
...
#         }
#         mock_response = MockAsyncDispatch(response_body)
#         requestor = RockRequestor(base_url=base_url, dispatch=mock_response)
#         assert requestor.is_authenticated is False
#         requestor.cookies.set(
#             **cookie
#         )  # manually set the cookie to simulate a response
#         await requestor.authenticate()
#         assert requestor.is_authenticated is True

#     async def test_authenticate_404(self, response_body):
#         mock_response = MockAsyncDispatch(response_body, status_code=404)
#         requestor = RockRequestor(base_url=base_url, dispatch=mock_response)
...
#         requestor = RockRequestor(base_url)
#         assert requestor.base_url == "http://127.0.0.1"
#         assert len(requestor.headers) == 0
#         assert len(requestor.params) == 0
